...
@section('content')
  <main>
    <!-- Breadcrumb Start -->
    <div x-data="{ pageName: '{{ $client->name }}' }">
        @include('partials.breadcrumb', ['pageName' => '{{ $client->name }}'])
      </div>
      <!-- Breadcrumb End -->
        ...
        <div class="mb-6 rounded-2xl border border-gray-200 p-5 dark:border-gray-800 lg:p-6">
            ...
            <div class="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-7 2xl:gap-x-32">
                ...
                <div>
                    ...
                    <p class="text-sm font-medium text-gray-800 dark:text-white/90">
                        {{ $client->status == 0 ? 'Inactive' : 'Active' }}
                    </p>
                </div>
            </div>
            </div>
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                @forelse ($client->brief as $brief)
                    <tr>
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <span class="block font-medium text-gray-800 text-theme-sm dark:text-white/90">
                                            <a href="{{ route('client.brief.show', ['client' => $client->id, 'brief_id' => $brief->id]) }}" class="hover:underline">{{ $brief->name }}</a>
                                        </span>
                                        <span class="block text-gray-500 text-theme-xs dark:text-gray-400">
                                            {{ $brief->category->name ?? 'N/A' }}
                                        </span>
                                    </div>
                                </div>
                            </div>
                        ...
                        <td class="py-3">
                            <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ \Carbon\Carbon::parse($brief->start_date)->format('d/m/Y') }}</p>
                        </td>
                        <td class="py-3">
                            <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ \Carbon\Carbon::parse($brief->end_date)->format('d/m/Y') }}</p>
                        </td>
                        <td class="py-3">
                            <p class="text-gray-500 text-theme-sm dark:text-gray-400">KES {{ number_format($brief->budget, 2) }}</p>
                        </td>
                        <td class="py-3">
                            <p class="text-gray-500 text-theme-sm dark:text-gray-400">KES {{ number_format($brief->budget, 2) }}</p>
                        </td>
                        <td class="py-3">
                            <p class="rounded-full {{ $brief->status == 0 ? 'bg-success-50 text-success-600' : 'bg-error-50 text-error-500' }} px-2 py-0.5 text-theme-xs font-medium text-center">{{ $brief->status == 0 ? 'Open' : 'Closed' }}</p>                        
                        </td>
                        <td class="py-3">
                            <a href="{{ route('briefs.brief', ['brief_id' => $brief->id]) }}" class="text-blue-600 hover:underline">
                        ...
                        <td class="py-3">
                            <a href="{{ route('briefs.edit', $brief->id) }}" class="text-green-600 hover:underline">
                            ...
                            <form action="{{ route('briefs.destroy', $brief->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure?');">
                                @csrf
                                @method('DELETE')
