...
@section('content')
    ...
    <div class="container mx-auto mb-5 p-6 bg-white dark:bg-gray-900 rounded-lg shadow">
        ...
        <div class="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-7 2xl:gap-x-32">
            ...
            <div>
                ...
                <p class="text-sm font-medium text-gray-800 dark:text-white/90">
                    KES {{ number_format($brief->budget, 2) }}
                </p>
            </div>
            ...
            <div>
                ...
                <p class="text-sm font-medium text-gray-800 dark:text-white/90">
                    KES {{ number_format($brief->production_cost, 2) }}
                </p>
            </div>
            ...
            <div>
                ...
                <p class="text-sm font-medium text-gray-800 dark:text-white/90">
                    KES {{ number_format($brief->budget - $brief->production_cost, 2) }}
                </p>
            </div>
            ...
            <div>
                ...
                <p class="text-sm font-medium text-gray-800 dark:text-white/90">
                    @if($brief->budget > 0)
                        {{ number_format(($brief->budget - $brief->production_cost) / $brief->budget * 100, 2) }}%
                    @else
                        N/A
                    @endif
                </p>
    ...
    <div class="flex flex-col gap-2 mb-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('clients.briefs.items.store', ['client' => $client->id, 'brief' => $brief->id]) }}">
                <button class="inline-flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-theme-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200">
                    Add Item
                </button>
            </a>
        ...
        <table class="min-w-full">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    ...
                    <tr>
                        <td class="py-3">
                            <span class="block font-medium text-gray-800 text-theme-sm dark:text-white/90">
                                <a href="{{ route('items.show', ['client_id' => $client->id, 'brief_id' => $brief->id, 'item_id' => $item->id]) }}" class="text-blue-600 hover:underline">
                                    {{ $item->name }}
                                </a>
                            </span>
                        ...
                        <td class="py-3">
                            <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ $item->category->name ?? 'N/A' }}</p>
                        </td>
                        <td class="py-3">
                            <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ $quantity }} {{ $item->unit }}s</p>
                        </td>
                        <td class="py-3">
                            <p class="text-gray-500 text-theme-sm dark:text-gray-400">KES {{ number_format($price, 2) }}</p>
                        </td>
                        <td class="py-3">
                            <p class="text-gray-500 text-theme-sm dark:text-gray-400">KES {{ number_format($total, 2) }}</p>
                        </td>
                        <td class="py-3">
                            <a href="{{ route('briefs.edit', $item->id) }}" class="text-green-600 hover:underline">
                            ...
                            <form action="{{ route('briefs.destroy', $item->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure?');">
                                @csrf
                                @method('DELETE')
                ...
                <tr class="bg-gray-100">
                    ...
                    <td class="py-2 px-4 border-b font-bold">KES {{ number_format($grandTotal, 2) }}</td>
                    <td class="py-2 px-4 border-b font-bold">{{ number_format($totalQuantity) }} items</td>
                </tr>
            </tfoot>
            </tbody>
