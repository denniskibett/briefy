...
@section('content')
    ...
    <div x-data="{ pageName: 'Suppliers' }">
        @include('partials.breadcrumb', ['pageName' => '{{ $supplier->name }}'])
      </div>
      <!-- Breadcrumb End -->
      ...
      <div class="w-full overflow-x-auto">
        <table class="min-w-full">
          ...
          <tr>
                  ...
                  <div class="flex items-center gap-3">
                    ...
                    <div>
                        <a href="{{ route('suppliers.show', $supplier->id) }}">
                                <p class="font-medium text-gray-800 text-theme-sm dark:text-white/90">{{ $supplier->name ?? 'N/A' }}</p>
                        </a>
                      <span class="text-gray-500 text-theme-xs dark:text-gray-400">{{ $supplier->contact_name }}</span>
                    </div>
                  </div>
                </div>
              </td>
              <td class="py-3">
                <div>
                  <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ $supplier->address }}</p>
                  <span class="text-gray-500 text-theme-xs dark:text-gray-400">{{ $supplier->location }}</span>
                </div>
              </td>
              <td class="py-3">
                <div>
                  <p class="text-gray-500 text-theme-sm dark:text-gray-400"> {{ $supplier->phone }}</p>
                  <span class="text-gray-500 text-theme-xs dark:text-gray-400">{{ $supplier->email }}</span>
                </div>
              </td>
              <td class="py-3">
                <div class="flex items-center">
              ...
              <td class="py-3">
                <div class="flex items-center">
                  <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ $supplier->category_id }}</p>
                </div>
              </td>
              
              <td class="py-3">
                    <a href="{{ route('suppliers.edit', $supplier->id) }}" class="text-green-600 hover:underline">
                    ...
                    <form action="{{ route('suppliers.destroy', $supplier->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure?');">
                        @csrf
                        @method('DELETE')
