...
@section('content')
  <main>
    <!-- Breadcrumb Start -->
    <div x-data="{ pageName: '{{ $supplier->name }} - Bill of Materials (BOM)' }">
        @include('partials.breadcrumb', ['pageName' => '{{ $supplier->name }} - Bill of Materials (BOM)'])
    </div>
    <!-- Breadcrumb End -->
      ...
      <div class="w-full overflow-x-auto">
        <table class="min-w-full">
          ...
          <tr>
              <td class="py-3">
                <div class="flex items-center">
                  <div class="flex items-center gap-3">
                    ...
                    <div>
                      <p class="font-medium text-gray-800 text-theme-sm dark:text-white/90">{{ $bom->name }}</p>
                      <span class="text-gray-500 text-theme-xs dark:text-gray-400">KES {{ number_format($bom->pivot->unit_cost, 2) }}/piece</span>
                    </div>
                  </div>
                </div>
              </td>
              <td class="py-3">
                <div class="flex items-center">
                  <p class="text-gray-500 text-theme-sm dark:text-gray-400">Laptop</p>
                </div>
              </td>
              <td class="py-3">
                <div class="flex items-center">
                  <p class="text-gray-500 text-theme-sm dark:text-gray-400">KES {{ $bom->pivot->quantity }}</p>
                </div>
              </td>
              <td class="py-3">
                <div class="flex items-center">
                  <p class="rounded-full bg-success-50 px-2 py-0.5 text-theme-xs font-medium text-success-600 dark:bg-success-500/15 dark:text-success-500">Delivered</p>
                </div>
              </td>
