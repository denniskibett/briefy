...
<main>
    <!-- Breadcrumb Start -->
    <div x-data="{ pageName: 'Add Supplier for {{ $item->name }}' }">
        @include('partials.breadcrumb', ['pageName' => 'Add Supplier'])
    </div>
    <!-- Breadcrumb End -->

    <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] lg:p-6">
        <h3 class="mb-5 text-lg font-semibold text-gray-800 dark:text-white/90 lg:mb-7">
            Add Supplier for - {{ $item->name }}
        </h3>

        <form action="{{ route('bom_supplier_items.store', ['client_id' => $client_id ?? 'default', 'brief_id' => $brief_id ?? 'default', 'item_id' => $item->id]) }}" method="POST">
            @csrf
            ...
            <div class="mb-6 rounded-2xl border border-gray-200 p-5 dark:border-gray-800 lg:p-6">
                <div class="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-7 2xl:gap-x-32">
                    ...
                    <div class="w-full">
                        ...
                        >
                            <option value="" disabled selected>-- Choose Bill of Material for {{ $item->name }} --</option>
                            @foreach ($bomSuppliers as $supplier)
                                <option value="{{ $supplier->id }}">
                                    (BoM: {{ $supplier->bom->name ?? 'No BoM Info' }} | 
                                    <strong>{{ $supplier->supplier->name ?? 'No Supplier' }}</strong>)
                                </option>
                            @endforeach
                        </select>
