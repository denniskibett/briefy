...
<main>
    ...
    <div x-data="{ pageName: 'Briefs' }">
        @include('partials.breadcrumb', ['pageName' => 'Add Items to Brief'])
    </div>
    <!-- Breadcrumb End -->

    <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] lg:p-6">
        <div class="px-5 py-4 sm:px-6 sm:py-5">
            <h3 class="text-base font-medium text-gray-800 dark:text-white/90">Add Items to Brief: {{$brief->name}}</h3>
        </div>

        <form action="{{ route('briefs.items.store', ['brief' => $brief->id]) }}" method="post" class="border-t border-gray-100 p-5 dark:border-gray-800 sm:p-6">
            @csrf
                ...
                <tbody>
                    @foreach ($items as $item)
                        <tr>
                            ...
                            <td class="p-2 border">{{ $item->MOU }}</td>
                            <td class="p-2 border">
