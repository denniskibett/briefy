    ...
    <div class="container mx-auto p-6 bg-white dark:bg-gray-900 rounded-lg shadow">
        ...
        <div class="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-7 2xl:gap-x-32">
            ...
            <div>
                ...
                <p class="text-sm font-medium text-gray-800 dark:text-white/90">{{ $brief->name }}</p>
            </div>

            <div>
                <p class="mb-2 text-xs leading-normal text-gray-500 dark:text-gray-400">Item</p>
                <p class="text-sm font-medium text-gray-800 dark:text-white/90">{{ $item->name }}</p>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-7">
                ...
                <div>
                    ...
                    <p class="text-sm font-medium text-gray-800 dark:text-white/90">
                        {{ $item->briefs->where('id', $brief->id)->first()->pivot->quantity ?? 'N/A' }}
                    </p>
                </div>
            </div>
        </div>
...
<br />
    ...
    <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white px-4 pb-3 pt-4 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6">
        <div class="flex flex-col gap-2 mb-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <a href="{{ route('bom_supplier_items.create', ['client_id' => $client->id, 'brief_id' => $brief->id, 'item_id' => $item->id]) }}">
                <button class="inline-flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-theme-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200">
                    Create BoM
                </button>
            </a>
    ...
    @if($item->bomSupplierItems->isEmpty())
        ...
        <div class="w-full overflow-x-auto">
            <table class="min-w-full">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    @foreach($item->bomSupplierItems as $bomSupplierItem)
                        <tr>
                            <td class="py-3">
                                <p class="text-gray-500 text-theme-sm dark:text-gray-400">
                                    {{ $bomSupplierItem->bomSupplier->bom->name ?? 'N/A' }}
                                </p>
                            </td>
                            <td class="py-3">
                                <a href="{{ route('suppliers.show', $bomSupplierItem->bomSupplier->supplier->id) }}" class="text-blue-600 hover:underline">
                                    {{ $bomSupplierItem->bomSupplier->supplier->name ?? 'N/A' }}
                                </a>
                            </td>
                            <td class="py-3">
                                <p class="text-gray-500 text-theme-sm dark:text-gray-400">KES {{ number_format($bomSupplierItem->bomSupplier->unit_cost, 2) }}</p>
                            </td>
                            <td class="py-3">
                                <p class="text-gray-500 text-theme-sm dark:text-gray-400">
                                    {{ $item->briefs->where('id', $brief->id)->first()->pivot->quantity ?? 'N/A' }}
                            ...
                            <td class="py-3">
                                <p class="text-gray-500 text-theme-sm dark:text-gray-400">
                                    KES {{ number_format($bomSupplierItem->bomSupplier->unit_cost * ($item->briefs->where('id', $brief->id)->first()->pivot->quantity ?? 0), 2) }}
                                </p>
                            </td>
                            <td class="py-3">
                                <div class="flex gap-3">
                                    <!-- Edit Button -->
                                    <a href="{{ route('bom_supplier_items.edit', ['client_id' => $client->id, 'brief_id' => $brief->id, 'item_id' => $item->id, 'id' => $bomSupplierItem->id]) }}" class="text-green-600 hover:underline">
                                    ...
                                    <!-- Delete Button -->
                                    <form action="{{ route('bom_supplier_items.destroy', ['client_id' => $client->id, 'brief_id' => $brief->id, 'item_id' => $item->id, 'id' => $bomSupplierItem->id]) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this item?');">
                                        @csrf
                                        @method('DELETE')
                ...
                <tfoot>
                    <tr class="bg-gray-100 dark:bg-gray-800">
                        ...
                        <td class="py-2 px-4 border-t font-bold">
                            KES {{ number_format($item->bomSupplierItems->sum(function($bomSupplierItem) use ($brief, $item) {
