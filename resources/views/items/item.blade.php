    ...
    <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white px-4 pb-3 pt-4 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6">
      <div class="flex flex-col gap-2 mb-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex items-center gap-3">
          ...
          <a href="{{ route('bom_supplier_items.add', $item->id) }}">
            <button class="inline-flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-theme-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200">
                + Add BOM Supplier
            </button>
            </a>
      ...
      <div class="w-full overflow-x-auto">
        <table class="min-w-full">
          ...
          @forelse ($item->bomSupplierItems as $bomItem)
          ...
          <tr>
              <td class="py-3">
                <div class="flex items-center">
                  <div class="flex items-center gap-3">
                    ...
                    <div>
                        <a href="#">
                            <p class="font-medium text-gray-800 text-theme-sm dark:text-white/90">{{ $bomItem->bomSupplier->bom->name }}</p>
                            <span class="text-gray-500 text-theme-xs dark:text-gray-400">{{ $item->category->name }}</</span>
                        </a>
                    </div>
                  </div>
                </div>
              ...
              <td class="py-3">
                <div>
                  <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ $bomItem->bomSupplier->supplier->name }}</p>
                  <span class="text-gray-500 text-theme-xs dark:text-gray-400">{{ $bomItem->bomSupplier->supplier->contact_name }}</span>
                </div>
              </td>
              <td class="py-3">
                <div>
                  <p class="text-gray-500 text-theme-sm dark:text-gray-400"></p>
                  <span class="text-gray-500 text-theme-xs dark:text-gray-400">KES {{ $bomItem->bomSupplier->unit_cost }}</span>
                </div>
              </td>
            ...
            @empty
            <tr>
                No suppliers found for {{ $bom_name ?? 'BOM' }}. Add a supplier now
            </tr>
            @endforelse
