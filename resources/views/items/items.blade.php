    ...
    <div class="container mx-auto">
        ...
        <div class="{{ $tableConfig['container'] }}">
            <div class="{{ $tableConfig['headerWrapper'] }}">
                <div>
                    <h3 class="{{ $tableConfig['headerTitle'] }}">Item List ({{ $totalItems }})</h3>
                </div>
                    ...
                    <thead>
                        <tr class="{{ $tableConfig['theadRow'] }}">
                            <th class="{{ $tableConfig['th'] }}">
                                <p class="{{ $tableConfig['thText'] }}">Item Name</p>
                            </th>
                            <th class="{{ $tableConfig['th'] }}">
                                <p class="{{ $tableConfig['thText'] }}">Category</p>
                            </th>
                            <th class="{{ $tableConfig['th'] }}">
                                <p class="{{ $tableConfig['thText'] }}">MOU</p>
                            </th>
                            <th class="{{ $tableConfig['th'] }}">
                            ...
                            <tr>
                                ...
                                <td class="py-3">
                                    <span class="block font-medium text-gray-800 text-theme-sm dark:text-white/90">
                                        <a href="{{ route('items.item', $item->id) }}" class="hover:underline">
                                            {{ $item->name }}
                                        </a>
                                    </span>
                                </td>
                                ...
                                <!-- Actions Column -->
                                <td class="py-3">
                                    <a href="{{ route('items.edit', $item->id) }}" class="text-green-600 hover:underline">
